...
set application_status = '承認済'
where application_status is null;

alter table public.member
  alter column application_status set default '未審査';
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'member_application_status_check'
  ) then
    alter table public.member
      add constraint member_application_status_check
      check (application_status in ('未審査', '承認済', '差戻し', '却下'));
  end if;
end
$$;

create index if not exists member_application_status_idx
  on public.member (application_status, application_submitted_at desc);
...
grant execute on function public.cidm_submit_application(jsonb) to service_role;
